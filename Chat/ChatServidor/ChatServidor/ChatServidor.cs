...
namespace ChatServidor
{
    ...
    class ChatServidor
    {
        ...
        public static void EnviaMensagemAdmin(string Mensagem)
        {
            ...
            String[] usuarios = new String[ChatServidor.htUsuarios.Count];
            ChatServidor.htUsuarios.Values.CopyTo(tcpClientes, 0);
            ChatServidor.htUsuarios.Keys.CopyTo(usuarios, 0);
        ...
        public static void EnviaMensagem(string Origem, string Mensagem, string Endereco)
        {
            StreamWriter swSenderSender;

            e = new StatusChangedEventArgs(Origem + " disse para " + Endereco + ": " + Mensagem);
            OnStatusChanged(e);
            ...
            for (int i = 0; i < tcpClientes.Length; i++)
            {
                try
                {
                    ...
                    {
                        swSenderSender = new StreamWriter(tcpClientes[i].GetStream());
                        swSenderSender.WriteLine(Origem + " disse para " + Endereco + ": " + Mensagem);
                        swSenderSender.Flush();
                        swSenderSender = null;
                    }
                }
        ...
        private void AceitaCliente()
        {
            ...
            try
            {
                while ((strResposta = srReceptor.ReadLine()) != "")
                {
                    ...
                    {
                        var stResp = strResposta.Split('§');
                        ChatServidor.EnviaMensagem(usuarioAtual, stResp[0], stResp[1]);
                    }
                }
            }
            catch
